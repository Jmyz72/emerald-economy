...
public class SellCommands {
    ...
    public static void register(CommandDispatcher<ServerCommandSource> dispatcher) {
        dispatcher.register(CommandManager.literal("worth")
                .requires(source -> savage.emeraldeconomy.util.PermissionsHelper.check(source, "emeraldeconomy.command.worth", true))
        ...
        dispatcher.register(CommandManager.literal("sell")
                .requires(source -> savage.emeraldeconomy.util.PermissionsHelper.check(source, "emeraldeconomy.command.sell", true))
    ...
    private static int listWorth(CommandContext<ServerCommandSource> context) {
        Map<String, savage.emeraldeconomy.config.ItemPrice> prices = EconomyManager.getInstance().getAllItemPrices();
        if (prices.isEmpty()) {
        ...
        for (Map.Entry<String, savage.emeraldeconomy.config.ItemPrice> entry : prices.entrySet()) {
            savage.emeraldeconomy.config.ItemPrice p = entry.getValue();
    ...
    private static int sellHand(CommandContext<ServerCommandSource> context) throws CommandSyntaxException {
        ...
        savage.emeraldeconomy.economy.TradeService.Result r =
                savage.emeraldeconomy.economy.TradeService.sell(player, itemId, stack.getCount());
    ...
    private static int sellItem(CommandContext<ServerCommandSource> context, int maxAmount) throws CommandSyntaxException {
        ...
        savage.emeraldeconomy.economy.TradeService.Result r =
                savage.emeraldeconomy.economy.TradeService.sell(player, itemId, maxAmount);
        return reportSell(context, r, itemId);
    }

    private static int reportSell(CommandContext<ServerCommandSource> context,
                                  savage.emeraldeconomy.economy.TradeService.Result r, String itemId) {
        switch (r.status()) {
