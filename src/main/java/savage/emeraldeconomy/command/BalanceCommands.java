...
public class BalanceCommands {
    ...
    public static void register(CommandDispatcher<ServerCommandSource> dispatcher) {
        dispatcher.register(CommandManager.literal("bal")
                .requires(source -> savage.emeraldeconomy.util.PermissionsHelper.check(source, "emeraldeconomy.command.bal", true))
                .executes(BalanceCommands::checkSelfBalance)
                .then(CommandManager.argument("target", StringArgumentType.string())
                        .requires(source -> savage.emeraldeconomy.util.PermissionsHelper.check(source, "emeraldeconomy.command.bal.others", true))
        ...
        dispatcher.register(CommandManager.literal("baltop")
                .requires(source -> savage.emeraldeconomy.util.PermissionsHelper.check(source, "emeraldeconomy.command.baltop", true))
                .executes(BalanceCommands::balTop));

        dispatcher.register(CommandManager.literal("pay")
                .requires(source -> savage.emeraldeconomy.util.PermissionsHelper.check(source, "emeraldeconomy.command.pay", true))
                .then(CommandManager.argument("target", StringArgumentType.string())
                        .suggests(CommandSupport.PLAYER_SUGGESTION_PROVIDER)
        ...
        dispatcher.register(CommandManager.literal("withdraw")
                .requires(source -> savage.emeraldeconomy.util.PermissionsHelper.check(source, "emeraldeconomy.command.withdraw", true))
    ...
    private static int pay(CommandContext<ServerCommandSource> context) throws CommandSyntaxException {
        ...
        savage.emeraldeconomy.util.TransactionLogger.log("PAY", sourcePlayer.getName().getString(), displayName, amount, "Payment");
        return 1;
    }
