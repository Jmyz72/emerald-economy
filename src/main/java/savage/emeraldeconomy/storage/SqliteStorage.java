...
import savage.emeraldeconomy.EconomyManager.AccountData;
...
public class SqliteStorage extends savage.savdbcore.storage.SqlStorage implements EconomyStorage {
    private final savage.emeraldeconomy.EconomyManager manager;

    public SqliteStorage(savage.emeraldeconomy.EconomyManager manager, String tablePrefix) {
        super(tablePrefix);
        ...
        try (Connection conn = getConnection()) {
            ...
            try (PreparedStatement checkStmt = conn.prepareStatement("SELECT version FROM " + tablePrefix + "accounts LIMIT 1")) {
                ...
                try (PreparedStatement alterStmt = conn.prepareStatement("ALTER TABLE " + tablePrefix + "accounts ADD COLUMN version BIGINT DEFAULT 0")) {
                    ...
                    EmeraldEconomy.LOGGER.error("Failed to add 'version' column during migration", ex);
                }
            }
            ...
            EmeraldEconomy.LOGGER.error("Failed to create economy tables", e);
    ...
    public BigDecimal getBalance(UUID uuid) {
        try (Connection conn = getConnection();
            ...
            EmeraldEconomy.LOGGER.error("Failed to read balance for {}", uuid, e);
    ...
    public void setBalance(UUID uuid, BigDecimal amount) {
        try (Connection conn = getConnection();
            ...
            EmeraldEconomy.LOGGER.error("Failed to set balance for {}", uuid, e);
    ...
    public boolean setBalance(UUID uuid, BigDecimal amount, long expectedVersion) {
        try (Connection conn = getConnection();
            ...
            EmeraldEconomy.LOGGER.error("Failed to set balance (versioned) for {}", uuid, e);
            throw new EconomyStorageException("Failed to set balance for " + uuid, e);
        }
    }
    ...
    public boolean hasAccount(UUID uuid) {
        try (Connection conn = getConnection();
            ...
            EmeraldEconomy.LOGGER.error("Failed to check account for {}", uuid, e);
    ...
    public AccountData getAccount(UUID uuid) {
        try (Connection conn = getConnection();
            ...
            EmeraldEconomy.LOGGER.error("Failed to read account for {}", uuid, e);
    ...
    public void createAccount(UUID uuid, String name) {
        if (hasAccount(uuid)) {
            try (Connection conn = getConnection();
                ...
                EmeraldEconomy.LOGGER.error("Failed to update account name for {}", uuid, e);
            ...
            try (Connection conn = getConnection();
                ...
                EmeraldEconomy.LOGGER.error("Failed to create account for {}", uuid, e);
    ...
    public UUID getUUID(String name) {
        try (Connection conn = getConnection();
            ...
            EmeraldEconomy.LOGGER.error("Failed to look up UUID for {}", name, e);
    ...
    public Collection<String> getOfflinePlayerNames() {
        ...
        try (Connection conn = getConnection();
            ...
            EmeraldEconomy.LOGGER.error("Failed to list offline player names", e);
        }
        return names;
    }
    ...
    public void logTransaction(long timestamp, String source, String target, BigDecimal amount, String type, String details) {
        try (Connection conn = getConnection();
            ...
            EmeraldEconomy.LOGGER.error("Failed to log transaction ({} -> {})", source, target, e);
            throw new EconomyStorageException("Failed to log transaction", e);
        }
    }

    @Override
    public List<savage.emeraldeconomy.util.TransactionLogger.LogEntry> searchLogs(String target, long cutoffTimestamp) {
        List<savage.emeraldeconomy.util.TransactionLogger.LogEntry> logs = new ArrayList<>();
        ...
        try (Connection conn = getConnection();
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                while (rs.next()) {
                    logs.add(new savage.emeraldeconomy.util.TransactionLogger.LogEntry(
            ...
            EmeraldEconomy.LOGGER.error("Failed to search transaction logs", e);
        }
        return logs;
    }
    ...
    public List<AccountData> getTopAccounts(int limit) {
        ...
        try (Connection conn = getConnection();
            ...
            EmeraldEconomy.LOGGER.error("Failed to read top accounts", e);
        }
        return accounts;
    }
    ...
    public void deleteAccount(UUID uuid) {
        try (Connection conn = getConnection();
            ...
            EmeraldEconomy.LOGGER.error("Failed to delete account for {}", uuid, e);
